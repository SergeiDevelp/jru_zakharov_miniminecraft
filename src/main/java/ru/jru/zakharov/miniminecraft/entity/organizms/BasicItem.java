...
public abstract class BasicItem implements Reproducible, Cloneable {
    ...
    public BasicItem(String name, String icon, Limit limit) {
        this.name = name;
        this.icon = icon;
        this.limit = limit;
        weight = Randomaizer.random(limit.getMaxWeight() / 2, limit.getMaxWeight());
    }
    ...
    @Override
    protected BasicItem clone() throws CloneNotSupportedException {
        //visible in inherits (cast to Organism)
        BasicItem clone = (BasicItem) super.clone();
        clone.id = idCounter.incrementAndGet();
        clone.weight = Randomaizer.random(limit.getMaxWeight() / 2, limit.getMaxWeight());
        return clone;
    }

    @SuppressWarnings("unchecked")
    public static <T extends BasicItem> T clone(T original) {
    ...
    protected boolean safeAddTo(Cell cell) {
        cell.getLock().lock();
        try {
            Set<BasicItem> set = cell.getResidents().get(getType());
        ...
        try {
            ...
            if (!(needFood <= 0)) {
                ...
                while (needFood > 0 && iterator.hasNext()) {
                    ...
                    if (foods.size() > 0 && probably > Randomaizer.random(0, 100)) {
                        for (Iterator<BasicItem> organismIterator = foods.iterator(); organismIterator.hasNext(); ) {
                            BasicItem o = organismIterator.next();
